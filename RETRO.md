...
### Sprint 5 — AI Chatbot, Wishlist & REST API
- **What worked**: Modular ChatProvider interface with Gemini API and Mock fallback, per-session rate limiting, question caching, and Wishlist save-for-later.
- **What didn't**: Outbound network requests to external LLM could hang without strict socket timeouts.
- **One change for next sprint**: Always set strict connect/read timeouts (5000ms) with static degraded fallback.
